...
def main():
    ...
    N = Z.shape[0]
    D = Z.shape[1]
    Sigma = (Xc.T @ Xc) / max(1, N - 1) + args.shrinkage * torch.eye(D)

    # Build view slices and latent shapes (for decoding later)
    with torch.no_grad():
        shapes_by_view = []
        for vi, m in enumerate(models):
            # infer per-level shapes
            shapes = get_latent_shapes(m, to01(views[vi]).float())
            shapes_by_view.append(shapes)
        # compute flat dims & slices
        start = 0
    ...
    with open(metrics_path, "w", newline="") as fcsv:
        w = csv.writer(fcsv)
        w.writerow(["observed","missing","N","PSNR_mean","PSNR_std","MSE_mean","MSE_std"])
        ...
        for obs, mis in perms:
            ...
            w.writerow([ "+".join(obs), "+".join(mis), len(psnrs), f"{PS:.4f}", f"{PSs:.4f}", f"{ME:.6f}", f"{MEs:.6f}" ])
