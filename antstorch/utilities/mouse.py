...
def mouse_brain_extraction(image,
    ...
    Ported from antspynet.utilities.mouse_brain_extraction, using ANTsTorch's
    generic U-Net (2-D/3-D) architectures.
    ...
    Example
    -------
    >>> output = mouse_brain_extraction(mri_image)
    """
    ...
    if modality == "t2":
        ...
        template = ants.image_read(get_antstorch_data("bsplineT2MouseTemplate"))
        template = ants.resample_image(template, template_shape, use_voxels=True, interp_type=0)
        template_mask = ants.image_read(get_antstorch_data("bsplineT2MouseTemplateBrainMask"))
        ...
        weights_file_name = get_pretrained_network("mouseT2wBrainExtraction3D_pytorch",
            target_file_name="mouseT2wBrainExtraction3D_pytorch.pt")
        model.load_state_dict(_load_state_dict(weights_file_name), strict=True)
        model.eval()
        model = model.to(device)

        batchX = np.zeros((1, 1, *template_shape), dtype=np.float32)
        batchX[0, 0, :, :, :] = image_warped.numpy().astype(np.float32)
        ...
        with torch.no_grad():
            ...
            batchY = model(x).squeeze(0).squeeze(0).cpu().numpy()
        ...
        if "coronal" in modality.lower():
            weights_file_name = get_pretrained_network("ex5_coronal_weights_pytorch",
                target_file_name="ex5_coronal_weights_pytorch.pt")
        elif "sagittal" in modality.lower():
            weights_file_name = get_pretrained_network("ex5_sagittal_weights_pytorch",
                target_file_name="ex5_sagittal_weights_pytorch.pt")
        else:
        ...
        batch_X = np.zeros((number_of_channels * number_of_slices, 1, *resampled_image_size), dtype=np.float32)
        ...
        for i in range(number_of_channels):
            ...
            for j in range(number_of_slices):
                ...
                if slice.max() > slice.min():
                    ...
                    slice_array = slice_resampled.numpy().astype(np.float32)
        ...
        predicted_data = np.transpose(predicted_data, (0, 2, 3, 1))
...
def mouse_brain_parcellation(image,
    ...
    Ported from antspynet.utilities.mouse_brain_parcellation.
    ...
    mask : ANTsImage
        Brain mask.  If not specified, one is estimated using ANTsTorch mouse brain
        extraction.
    ...
    Example
    -------
    >>> output = mouse_brain_parcellation(image, which_parcellation="nick")
    """
    ...
    if (which_parcellation == "nick" or
        ...
        if which_parcellation == "nick":
            ...
            template = ants.image_read(get_antstorch_data("DevCCF_P56_MRI-T2_50um"))
            template_match = ants.rank_intensity(template)
            template_mask = ants.image_read(get_antstorch_data("DevCCF_P56_MRI-T2_50um_BrainParcellationNickMask"))
            weights_file_name = get_pretrained_network("mouseT2wBrainParcellation3DNick_pytorch",
                target_file_name="mouseT2wBrainParcellation3DNick_pytorch.pt")
        elif which_parcellation == "tct":
            template_string = "DevCCF P56 T2w"
            template = ants.image_read(get_antstorch_data("DevCCF_P56_MRI-T2_50um"))
            template_match = ants.rank_intensity(template)
            template_mask = ants.image_read(get_antstorch_data("DevCCF_P56_MRI-T2_50um_BrainParcellationTctMask"))
            weights_file_name = get_pretrained_network("mouseT2wBrainParcellation3DTct_pytorch",
                target_file_name="mouseT2wBrainParcellation3DTct_pytorch.pt")
        elif which_parcellation == "jay":
            template_string = "DevCCF P04 STPT"
            template = ants.image_read(get_antstorch_data("DevCCF_P04_STPT_50um"))
            template_match = ants.histogram_equalize_image(template)
            template_mask = ants.image_read(get_antstorch_data("DevCCF_P04_STPT_50um_BrainParcellationJayMask"))
            weights_file_name = get_pretrained_network("mouseSTPTBrainParcellation3DJay_pytorch",
                target_file_name="mouseSTPTBrainParcellation3DJay_pytorch.pt")
        ...
        if mask is None:
            ...
            mask = mouse_brain_extraction(image, modality="t2", device=device,
                                          verbose=verbose)
        ...
        batchX = np.zeros((1, channel_size, *template.shape), dtype=np.float32)
        batchX[0, 0, :, :, :] = image_warped.numpy().astype(np.float32)
        for i in range(len(template_priors)):
            batchX[0, i + 1, :, :, :] = template_priors[i].numpy().astype(np.float32)
        ...
        for i in range(number_of_classification_labels):
            ...
            probability_image = ants.from_numpy(y[i], origin=template.origin,
...
def mouse_cortical_thickness(t2,
    ...
    for segmentation.  Description concerning implementation and evaluation:
    ...
    return_isotropic_output : boolean
        The underlying parcellation network actually learns an interpolating function
...
def mouse_histology_brain_mask(image,
    ...
    weights_file_name = get_pretrained_network("allen_brain_mask_weights_pytorch",
        target_file_name="allen_brain_mask_weights_pytorch.pt")
...
def mouse_histology_hemispherical_coronal_mask(image,
    ...
    weights_file_name = get_pretrained_network("allen_brain_leftright_coronal_mask_weights_pytorch",
        target_file_name="allen_brain_leftright_coronal_mask_weights_pytorch.pt")
    ...
    for i in range(number_of_channels):
        ...
        for j in range(number_of_slices):
            ...
            if slice.max() > slice.min():
                ...
                slice_array = slice_smoothed.numpy().astype(np.float32)
                slice_array = (slice_array - slice_array.min()) / (slice_array.max() - slice_array.min())
                batch_X[count, 0, :, :] = slice_array
            count = count + 1
...
def mouse_histology_cerebellum_mask(image,
    ...
    if view == "sagittal":
        weights_file_name = get_pretrained_network("allen_cerebellum_sagittal_mask_weights_pytorch",
            target_file_name="allen_cerebellum_sagittal_mask_weights_pytorch.pt")
    elif view == "coronal":
        weights_file_name = get_pretrained_network("allen_cerebellum_coronal_mask_weights_pytorch",
            target_file_name="allen_cerebellum_coronal_mask_weights_pytorch.pt")
    else:
...
def mouse_histology_super_resolution(image,
    ...
    weights_file_name = get_pretrained_network("allen_sr_weights_pytorch",
        target_file_name="allen_sr_weights_pytorch.pt")
    ...
    batch_X = np.zeros((len(image_lr_list), 3, *lr_image_size), dtype=np.float32)
    for i in range(len(image_lr_list)):
        batch_X[i, :, :, :] = np.transpose(image_lr_list[i].numpy().astype(np.float32), (2, 0, 1))
