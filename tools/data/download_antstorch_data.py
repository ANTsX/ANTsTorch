...
def download_data(strict=False, cache_dir=None, data_keys=None, model_keys=None, verbose=False):
    ...
    """
    print("Downloading data files from get_antstorch_data...")
    if cache_dir is not None:
        antstorch.set_antstorch_cache_directory(cache_dir)
        print(f"Using custom cache directory: {cache_dir}")
    try:
        if data_keys is None:
            data_keys = antstorch.get_antstorch_data("show")
        for key in data_keys:
            if key == "show":
                continue
            try:
                print(f"  ↳ Downloading data: {key}")
                fpath = antstorch.get_antstorch_data(key, show_progress=verbose)
                if verbose:
    ...
    try:
        if model_keys is None:
            model_keys = antstorch.get_pretrained_network("show")
        for key in model_keys:
            if key == "show":
                continue
            try:
                print(f"  ↳ Downloading model: {key}")
                fpath = antstorch.get_pretrained_network(key, show_progress=verbose)
                if verbose:
                    print(f"    ✓ Saved to: {fpath}")
            except Exception as e:
                print(f"    ✗ Failed to download {key}: {e}")
...
if __name__ == "__main__":
    ...
    Download ANTsTorch data and / or pretrained models to the local cache directory.
